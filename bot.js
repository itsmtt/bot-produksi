...
client.on('message', async msg => {
    ...
    if (text.startsWith('!ubah')) {
        ...
        if (index === -1) return msg.reply(`⚠️ Data dengan ID ${id} tidak ditemukan.`);
...
🔢 Qty: ${entry.qty}`);
    ...
    if (text.startsWith('!hapus')) {
        ...
        data.splice(index, 1);
        saveData(data);
        return msg.reply(`🗑️ Data dengan ID ${id} (${entry.line} - ${entry.produk}) berhasil dihapus.`);
    }
    ...
    if (text.startsWith('!export')) {
        ...
        const tanggal = arg ? (mode === 'hari' ? arg.split('-').reverse().join('-') : arg) : (mode === 'hari' ? new Date().toISOString().slice(0, 10) : new Date().toISOString().slice(0, 7));
